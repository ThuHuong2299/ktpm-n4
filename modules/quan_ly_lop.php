...
<?php if ($action === 'edit' && $edit_class): ?>
    ...
    <?php if ($thong_bao_update): ?>
        <script>showSuccess('<?= addslashes($thong_bao_update) ?>');</script>
    <?php endif; ?>
    
    <?php if ($loi_update): ?>
        <script>showError('<?= addslashes($loi_update) ?>');</script>
    <?php endif; ?>
        ...
        <table id="gradeTable">
            ...
            <tbody id="tableBody">
                <?php if (!empty($classes)): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="7" class="empty-message">Chưa có lớp học phần nào trong hệ thống.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
