...
if ($class_id && $class_info) {
    try {
        ...
        // Kiểm tra nếu không có sinh viên nào trong lớp
        if (empty($student_grades)) {
            $error_message = "Không có sinh viên nào trong lớp học phần này.";
        }
...
// Xử lý tạo báo cáo
if (isset($_POST['generate_report']) && $class_id) {
    $format = $_POST['format'] ?? 'pdf';
    
    try {
        // Kiểm tra lại điều kiện xuất báo cáo
        $stmt = $pdo->prepare("
            SELECT COUNT(*) as completed_students
            FROM Diem d 
            JOIN SinhVien_LopHocPhan slhp ON d.MaSinhVien = slhp.MaSinhVien AND d.MaLopHocPhan = slhp.MaLopHocPhan
        ...
        if ($completed_count > 0) {
            $success_message = "Đang tạo báo cáo {$format} cho {$completed_count} sinh viên. Báo cáo sẽ được tải xuống sau vài giây.";
            // Ở đây sẽ thêm logic tạo báo cáo thực tế
        } else {
            $error_message = "Không thể tạo báo cáo: Chưa có sinh viên nào có điểm tổng kết hoàn chỉnh.";
        }
    } catch (PDOException $e) {
        $error_message = "Lỗi khi tạo báo cáo: " . $e->getMessage();
    }
}

// Hiển thị thông báo qua notification system
if ($success_message): ?>
...
<?php if ($class_info): ?>
    ...
    <div style="margin-top: 20px; background: #f8f9fa; padding: 15px; border: 1px solid #ddd; border-radius: 4px;">
        ...
        <?php if ($export_allowed): ?>
            <form method="post" style="display: flex; gap: 10px; align-items: center;" onsubmit="return confirmReportGeneration(this);">
                <select name="format" class="form-control" style="width: 150px;">
                    <option value="pdf">PDF</option>
                    <option value="excel">Excel</option>
                </select>
                <button type="submit" name="generate_report" class="btn btn-primary" id="generateReportBtn">Tạo báo cáo</button>
            </form>
        <?php else: ?>
...
<?php else: ?>
    ...
    <div style="background: #f8f9fa; padding: 20px; border: 1px solid #ddd; border-radius: 4px; text-align: center;">
        ...
        <form method="GET" action="index.php" class="search-form" style="justify-content: center;" onsubmit="return validateClassSelection(this);">
            <input type="hidden" name="page" value="thong_ke">
            <select name="class" id="classSelect" style="flex: 0 0 400px; padding: 10px; border: 1px solid #ddd; border-radius: 4px;" required>
                ...
                try {
                    ...
                    if ($class_count === 0) {
                        echo '<option value="">Không có lớp học phần nào đang hoạt động</option>';
                        echo '<script>showWarning("Hiện tại không có lớp học phần nào đang hoạt động để xem thống kê.");</script>';
                    }
                } catch (PDOException $e) {
                    echo '<option value="">Lỗi tải danh sách lớp</option>';
                    echo '<script>showError("Lỗi khi tải danh sách lớp học phần: ' . addslashes($e->getMessage()) . '");</script>';
                }
                ?>
            </select>
            <button type="submit" id="viewStatsBtn">Xem thống kê</button>
        </form>
    </div>
<?php endif; ?>

<script>
// Validation cho form chọn lớp
function validateClassSelection(form) {
    const classSelect = document.getElementById('classSelect');
    const selectedValue = classSelect.value.trim();
    
    if (!selectedValue) {
        showError('Vui lòng chọn một lớp học phần để xem thống kê.');
        return false;
    }
    
    // Hiển thị loading message
    const submitBtn = document.getElementById('viewStatsBtn');
    if (submitBtn) {
        submitBtn.disabled = true;
        submitBtn.textContent = 'Đang tải...';
        showInfo('Đang tải dữ liệu thống kê, vui lòng đợi...');
    }
...
function confirmReportGeneration(form) {
    const formatSelect = form.querySelector('select[name="format"]');
    const format = formatSelect.value;
    const formatText = format === 'pdf' ? 'PDF' : 'Excel';
    
    if (!confirm(`Bạn có muốn tạo báo cáo ${formatText} cho lớp học phần này không?`)) {
        return false;
    }
    ...
    if (submitBtn) {
        ...
        showInfo(`Đang tạo báo cáo ${formatText}, vui lòng đợi...`);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php if ($class_info && !empty($student_grades)): ?>
        const totalStudents = <?= $statistics['total_students'] ?>;
        const gradedStudents = <?= $statistics['graded_students'] ?>;
        const passRate = <?= $statistics['pass_rate'] ?>;
        
        if (totalStudents === 0) {
            showWarning('Lớp học phần này chưa có sinh viên nào đăng ký.');
        } else if (gradedStudents === 0) {
            showInfo(`Lớp có ${totalStudents} sinh viên nhưng chưa có sinh viên nào được nhập điểm.`);
        } else if (gradedStudents < totalStudents) {
            showWarning(`Còn ${totalStudents - gradedStudents} sinh viên chưa được nhập điểm đầy đủ.`);
        } else {
            showSuccess(`Đã tải thành công thống kê cho ${totalStudents} sinh viên (tỷ lệ đạt: ${passRate.toFixed(1)}%).`);
        }
    <?php endif; ?>
    
    // Khôi phục trạng thái button nếu có lỗi
    const viewStatsBtn = document.getElementById('viewStatsBtn');
    const generateReportBtn = document.getElementById('generateReportBtn');
    
    if (viewStatsBtn && viewStatsBtn.disabled) {
        setTimeout(() => {
            viewStatsBtn.disabled = false;
            viewStatsBtn.textContent = 'Xem thống kê';
        }, 3000);
    }
    ...
    if (generateReportBtn && generateReportBtn.disabled) {
        setTimeout(() => {
            ...
            generateReportBtn.textContent = 'Tạo báo cáo';
        }, 3000);
    }
});
